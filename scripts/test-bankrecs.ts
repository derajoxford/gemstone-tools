...
(async () => {
  ...
  // Use URL auth to avoid the 401 header issue on this host
  const client = new GraphQLClient('https://api.politicsandwar.com/graphql?api_key='+encodeURIComponent(apiKey));

  try {
    const data:any = await client.request(Q, { ids: [a.id] });
    console.log(JSON.stringify(data, null, 2));
    const rows = data?.alliances?.data?.[0]?.bankrecs ?? null;
    if (rows === null) {
