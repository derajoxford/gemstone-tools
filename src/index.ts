...
client.on('interactionCreate', async (i: Interaction) => {
  try {
    ...
    console.error(err);
    if ('isRepliable' in i && i.isRepliable()) await (i as any).reply({ content: 'Something went wrong.', ephemeral: true });
  }
});
...
async function handleWithdrawOpenButton(i: ButtonInteraction) {
  const alliance = await prisma.alliance.findFirst({ where: { guildId: i.guildId ?? '' } });
  ...
  const keys = haveKeys.slice(0, 5); // Discord modal max 5 inputs
  const modal = new ModalBuilder().setCustomId(`wd:req:${keys.join(',')}`).setTitle('💸 Request Withdrawal');
...
async function handleWithdrawModal(i: any) {
  const suffix = i.customId.split(':')[2] || '';
  const keys = suffix ? suffix.split(',').filter(Boolean) : [];
  const alliance = await prisma.alliance.findFirst({ where: { guildId: i.guildId ?? '' } });
  if (!alliance) return i.reply({ content: 'No alliance linked here.', ephemeral: true });
  const member = await prisma.member.findFirst({ where: { allianceId: alliance.id, discordId: i.user.id }, include: { balance: true } });
  ...
  for (const k of keys) {
    ...
    if (isNaN(num)) return i.reply({ content: `Invalid number for ${k}.`, ephemeral: true });
  ...
  if (!Object.keys(payload).length) return i.reply({ content: 'Nothing requested.', ephemeral: true });

  await submitWithdraw(i, alliance.id, member, payload);
}
          ...
          if (member) {
            ...
            try {
              ...
              for (const k of ORDER) {
                const val = toNum((r as any)[k]);
                if (val) lines.push(fmtLine(k, val));
              }
              const nationUrl = `https://politicsandwar.com/nation/id=${member.nationId}`;
              const embed = new EmbedBuilder()
