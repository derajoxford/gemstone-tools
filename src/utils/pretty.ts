...
export const RESOURCE_META: Record<ResourceKey, { emoji: string; color: number; label: string }> = {
...
export function resourceLabel(key: ResourceKey): string {
  const m = RESOURCE_META[key]; return `${m.emoji} ${m.label}`;
}
export function colorForDelta(delta: number, fallback = COLORS.blurple): number {
  if (delta > 0) return COLORS.green; if (delta < 0) return COLORS.red; return fallback;
}
export function discordRelative(ts: Date | number): string {
  const ms = typeof ts === "number" ? ts : ts.getTime(); const s = Math.floor(ms / 1000); return `<t:${s}:R>`;
}
