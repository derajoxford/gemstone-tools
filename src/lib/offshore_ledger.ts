...
// Must match the tag we use in send()
export const OFFSH_NOTE_TAG = "Gemstone Offsh";

// Get or create a ledger row
export async function getOrCreateLedger(allianceId: number, offshoreId: number): Promise<OffshoreLedger> {
  let row = await prisma.offshoreLedger.findUnique({ where: { allianceId_offshoreId: { allianceId, offshoreId } } });
  if (row) return row;
...
// Incrementally catch up using offshore’s latest bankrecs.
// Strategy: pull the latest N (e.g., 500), and apply only rows with id > lastSeenBankrecId
// that are bot-tagged and match the A<->Off pair. This is idempotent and cheap.
export async function catchUpLedgerForPair(allianceId: number, offshoreId: number, take = 500): Promise<OffshoreLedger> {
  ...
  // Compute aggregate delta from unseen rows
  ...
  for (const r of rows) {
    ...
    for (const k of RESOURCE_KEYS) {
      ...
      // A→O increases holding; O→A decreases holding
      delta[k] += isAtoO ? v : -v;
    }
  ...
  // Return fresh row
  return prisma.offshoreLedger.findUniqueOrThrow({ where: { allianceId_offshoreId: { allianceId, offshoreId } } });
}
